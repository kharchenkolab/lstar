...
**A green suite only proves the code works on the inputs the fixtures feed.** The escapes here were untested branches, not wrong logic: an all-sparse corpus never exercised a *dense* measure (a whole surface's measure-read was broken, unnoticed); a comparison never met the NaN real data carries. So when a read/compute path branches on an input axis — encoding `{dense,csc,csr}`, nullability/NaN, coverage `{full,partial}` — feed *every* value of it, on *every* surface that implements the path; and since CI is synthetic (above), the synthetic fixture must be **built to span those axes** — the real corpus that would cover them by accident isn't there. Then assert the real outcome: for a path that degrades silently (a caught exception → a fallback), assert the artifact was *written*, not merely that nothing threw. Cross-surface specifics: `docs/parity.md`.
